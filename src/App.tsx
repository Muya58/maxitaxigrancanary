...
export default function App() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors }, trigger } = useForm<FormData>({
    defaultValues: {
      clientName: '',
      clientPhone: '',
      dateTime: '',
  ...
  const [aiMessages, setAiMessages] = useState<{ role: 'user' | 'model'; parts: { text: string }[] }[]>([]);
  const [aiInput, setAiInput] = useState('');
  const [aiLoading, setAiLoading] = useState(false);
  const [destinationTip, setDestinationTip] = useState<string | null>(null);
  ...
  const askAi = async (e: React.FormEvent) => {
    ...
    setAiInput('');
    setAiMessages(prev => [...prev, { role: 'user', parts: [{ text: userMessage }] }]);
    setAiLoading(true);
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: `Eres el asistente inteligente de MaxiTaxiGranCanary. Tu misión es ayudar a los turistas en Gran Canaria.
              Información clave:
              - Teléfono/WhatsApp: +34 619 735 892.
              - Email: soporte@maxitaxigrancanary.com
              - Especialidad: Traslados al aeropuerto, grupos (hasta 8 plazas), excursiones.
              - Ubicación: Gran Canaria.
              - Ton de voz: Profesional, amable y servicial.
              
              Responde de forma concisa y amigable. Si te preguntan por precios, guíalos al formulario de reserva o diles que contacten por WhatsApp o email para un presupuesto exacto.`,
          temperature: 0.7,
        }
      });
      
      const aiText = response.text || "Lo siento, no pude procesar tu solicitud.";
      setAiMessages(prev => [...prev, { role: 'model', parts: [{ text: aiText }] }]);
    } catch (err) {
      console.error("AI Error:", err);
      setAiMessages(prev => [...prev, { role: 'model', parts: [{ text: "Lo siento, tengo problemas de conexión. ¿Puedes contactarnos por WhatsApp?" }] }]);
    } finally {
      setAiLoading(false);
    }
  };

  const getDestinationTip = async () => {
    if (!watchDestination || watchDestination.length < 5) return;
    
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Dame un consejo corto (1 frase) sobre qué ver o hacer en ${watchDestination} o cerca de ahí en Gran Canaria. Sé muy breve.`,
        config: {
          temperature: 0.7,
        }
      });
      setDestinationTip(response.text || null);
    } catch (e) {
    ...
    <div className="min-h-screen bg-bg-deep selection:bg-brand/30">
                        ...
                        <AnimatePresence>
                          {destinationTip && (
                            <motion.p 
                              ...
                              className="text-[10px] text-brand font-bold italic pl-4 border-l border-brand mt-2"
                            >
                              AI Tip: {destinationTip}
                            </motion.p>
                          )}
                        </AnimatePresence>
                      </div>
      ...
      <section id="destinos" className="py-32 bg-ink border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {[
              { name: "Maspalomas", img: "input_file_0.png", alt: "Traslado en taxi a Maspalomas, Gran Canaria - Dunas y Hoteles" },
              { name: "Las Palmas", img: "input_file_1.png", alt: "Transfer a Las Palmas de Gran Canaria - Capital y Puerto" },
              { name: "Puerto Rico", img: "input_file_2.png", alt: "Taxi a Puerto Rico Gran Canaria - Playa y Ocio" },
              { name: "Meloneras", img: "input_file_3.png", alt: "Servicio Premium a Meloneras - Hoteles de Lujo" },
              { name: "Arucas", img: "input_file_4.png", alt: "Transfer a Arucas Gran Canaria - Iglesia y Casco Histórico" },
              { name: "Agaete", img: "input_file_5.png", alt: "Traslado a Agaete - Puerto de las Nieves" },
              { name: "Mogán", img: "input_file_6.png", alt: "Taxi a Puerto de Mogán - La Venecia de Canarias" },
            ].map((loc, i) => (
              <motion.div 
                key={i}
                initial={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-20 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
            <p className="text-slate-700 text-[9px] font-bold tracking-widest">
              &copy; {new Date().getFullYear()} MAXITAXI GRAN CANARIA. OFFICIAL LICENSED SERVICE.
            </p>
      ...
      <div className="fixed bottom-6 right-6 z-100 flex flex-col items-end gap-4">
        <AnimatePresence>
          {isAiOpen && (
            <motion.div
              ...
              className="w-80 h-[450px] bg-bg-deep border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col"
            >
              <div className="p-4 border-b border-white/5 bg-white/5 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h5 className="text-white text-xs font-black uppercase tracking-widest">MaxiBOT</h5>
                    <div className="flex items-center gap-1">
                      <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                      <span className="text-[8px] text-slate-500 font-bold uppercase">Online / AI Powered</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth">
                {aiMessages.length === 0 && (
                  <div className="text-center py-10 px-4">
                    <p className="text-slate-500 text-xs font-medium leading-loose">
                      ¡Hola! Soy tu asistente de MaxiTaxiGranCanary. <br/>
                      ¿Tienes dudas sobre traslados, equipaje o quieres saber qué visitar en la isla?
                    </p>
                  </div>
                )}
                ...
                {aiLoading && (
                  <div className="flex justify-start">
                    <div className="bg-white/5 text-slate-500 p-3 rounded-2xl rounded-bl-none border border-white/5">
              ...
              <form onSubmit={askAi} className="p-4 border-t border-white/5 bg-white/2">
                <div className="relative">
                  <input
                    type="text"
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-4 pr-10 text-white text-xs outline-none focus:border-brand transition-all"
                  />
                  <button type="submit" className="absolute right-2 top-2 text-brand hover:brightness-110 transition-all">
                    <ArrowRight size={18} />
                  </button>
                </div>
              </form>
        ...
        <button 
          ...
          className={`w-14 h-14 rounded-2xl shadow-2xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 ${
            isAiOpen ? 'bg-white/10 text-white rotate-90' : 'bg-brand text-white'
          }`}
        >
          {isAiOpen ? <X size={28} /> : <Globe size={28} />}
          {!isAiOpen && (
            <div className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-bg-deep rounded-full animate-pulse" />
          )}
        </button>
      </div>
    </div>
